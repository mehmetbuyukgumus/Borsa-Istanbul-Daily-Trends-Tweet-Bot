...
def tweeting_increasing(browser):
    ...
    while counter < tweet_limit:
        for i1, i2 in zip(name_increasing[0:10], highest_price[0:10]):
            ...
            ActionChains(browser).move_to_element(el).click(el).send_keys(f"***23.11.2023*** BİST'TE GÜNÜN ŞAMPİYONLARI!!! 🏅 #{i1} %{i2} artış oranıyla günün yükselenleri arasında yerini aldı 🚀⚡️💥📈").perform()
            time.sleep(2)
...
def tweeting_decrasing(browser):
    ...
    counter = 0
    while counter < tweet_limit:
        for i1, i2 in zip(name_decreasing[0:10], least_price[0:10]):
            el = browser.find_element(By.XPATH, "/html/body/div[1]/div/div/div[2]/main/div/div/div/div[1]/div/div[3]/div/div[2]/div[1]/div/div/div/div[2]/div[1]/div/div/div/div/div/div/div/div/div/div/label/div[1]/div/div/div/div/div/div[2]/div/div/div/div")
            ActionChains(browser).move_to_element(el).click(el).send_keys(f"***23.11.2023*** Günü düşüşle kapatanlar 👎🏻 #{i1} %{i2} düşüş oranıyla günün düşenleri arasında yerini aldı ⬇️🆘📉").perform()
            time.sleep(2)
            send_button = browser.find_element(By.XPATH, "/html/body/div[1]/div/div/div[2]/main/div/div/div/div[1]/div/div[3]/div/div[2]/div[1]/div/div/div/div[2]/div[2]/div[2]/div/div/div/div[3]/div/span/span")
            time.sleep(1)
            send_button.click()
